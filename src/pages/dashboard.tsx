...
import { getDietaryGuidances } from 'apiSdk/dietary-guidances';
import { DietaryGuidanceInterface } from 'interfaces/dietary-guidance';

import { CommunicationListPage as ShowCaseList } from 'pages/communications';
...
function DashboardPage() {
  ...
  const dietaryGuidanceStat = useSWR<PaginatedInterface<DietaryGuidanceInterface>>(
    () => `/dietary-guidances`,
    () => getDietaryGuidances({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Dietary Guidances',
      result: dietaryGuidanceStat?.data?.totalCount,
      isLoading: dietaryGuidanceStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`communications`} passHref legacyBehavior>
                  <Button
